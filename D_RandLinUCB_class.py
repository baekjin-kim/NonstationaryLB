...
class DRandLinUCB(object):
    def __init__(self, d, delta, alpha, lambda_, s, l, gamma, name, sm, sigma_noise, verbose, option):
        """
    ...
    def select_arm(self, arms):
        ...
        if self.option == "couple":
            self.z = np.random.normal(0, 1, size=1)
            for (i, a) in enumerate(arms):
                a = a.features
                invcov_a = np.inner(self.invcov @ self.cov_squared @ self.invcov, a.T)
                ucb_s[i] = np.dot(self.hat_theta, a) + self.alpha * self.z * beta_t * np.sqrt(np.dot(a, invcov_a))            
        if self.option == "couple_opt":
            # Optimism : consider Truncated Normal distribution over [0, infty)
            self.z = np.abs(np.random.normal(0, 1, size=1))
            for (i, a) in enumerate(arms):
                a = a.features
                invcov_a = np.inner(self.invcov @ self.cov_squared @ self.invcov, a.T)
                ucb_s[i] = np.dot(self.hat_theta, a) + self.alpha * self.z * beta_t * np.sqrt(np.dot(a, invcov_a))
    ...
    def update_state(self, features, reward):
        ...
        if not self.sm:
            ...
            raise NotImplementedError("Method SM is not implemented for D-RandLinUCB")
