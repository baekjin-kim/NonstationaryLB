...
class DLinUCB(object):
    def __init__(self, d, delta, alpha, lambda_, s, l, gamma, name, sm, sigma_noise, verbose):
        ...
        # build attributes
        self.c_delta = self.dim * log(1 / self.delta)  # first term in square root
    ...
    def select_arm(self, arms):
        ...
        beta_t = const1 + self.sigma_noise *np.sqrt(self.c_delta + self.dim * np.log(1 + self.l**2 * (1-self.gamma2_t)/(self.dim *self.lambda_*(1 - self.gamma**2))))
