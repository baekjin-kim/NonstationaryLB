...
"""
    File description: Implementation of the Linear Thompson Sampling
"""
...
class PolicyLinTS(object):
    def __init__(self, d, delta, alpha, lambda_, s, l, name, sm, sigma_noise, verbose, omniscient=False):
        """
        param:
    ...
    def select_arm(self, arms):
        ...
        beta_t = self.const1 + self.sigma_noise * np.sqrt(self.c_delta + self.dim * log(1 + self.t * self.l**2 /
    ...
    def update_state(self, features, reward):
        ...
        self.hat_theta = np.inner(self.invcov, self.b) + self.alpha * np.dot(scipy.linalg.sqrtm(self.invcov).real, z)
        if self.verbose:
            print('AAt:', aat)
